...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['bookSeats'])) {
    ...
    $date = $_POST['date'];
    $time = $_POST['time'];
    $booking_datetime = "$date $time"; // Combining date and time
    $total_price = count(explode(',', $seats)) * 12000;

    $sql = "INSERT INTO bookings (name, email, phone, seats, total_price, booking_datetime) 
            VALUES ('$name', '$email', '$phone', '$seats', $total_price, '$booking_datetime')";
...
<head>
    ...
    <style>
        .bus-layout { display: grid; grid-template-columns: repeat(4, 60px); gap: 10px; margin: 20px 0; }
        .seat { width: 50px; height: 50px; text-align: center; line-height: 50px; border-radius: 5px; cursor: pointer; }
        .available { background-color: green; color: white; }
        .booked { background-color: red; color: white; pointer-events: none; }
    ...
    <form id="bookingForm" method="POST">
        ...
        <div class="bus-layout">
            ...
            foreach ($seats as $seat) {
                ...
                if ($seatCheck->num_rows > 0) { $status = "booked"; }
                echo "<div class='seat $status' data-seat='$seat'>$seat</div>";
            }
            ?>
        </div>

        <p><strong>Price per seat:</strong> ₹12,000</p>
    ...
    <script>
        ...
        seats.forEach(seat => {
            seat.addEventListener('click', () => {
                ...
                totalPriceEl.innerText = selectedSeats.length * 12000;
